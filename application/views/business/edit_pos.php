                    ...
                    <div class="card card-outline card-warning">
                        ...
                        <div class="card-body">
                            <form id="place_order_form" method="post" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <h4><i class="fa fa-info-circle"></i><u>Edit Sale</u></h4>
                                        Order Reference: <u><?= $order_profile->bill_no ?> </u>
                                        <br />
                                        Order Date: <u><?= date('d M Y', strtotime($order_profile->date_time)) ?></u>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-12 table-responsive">
                                        <table class="table table-bordered table-condensed" id="order_table">
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                    <tr id='row<?= $x ?>'>
                                                        <input type="hidden" name="is_editted[]" value="0" readonly required />
                                                        <input type="hidden" name="print_session[]" value="jipos" readonly required />
                                                        <td class="td">
                                                            <input name='item[]' type='hidden' class='rm form-control' id='item_<?= $x ?>' value='<?= $dt->item_id ?>' style='pointer-events: none;' required />
                                                            <input name='item_name[]' class='rm form-control' id='item_name<?= $x ?>' value="<?= $dt->item_name . ' - ' . $dt->barcode ?>" style='pointer-events: none;' required />
                                                        </td>
                                                        <td>
                                                            <input style="pointer-events: none;" class='rm form-control' onkeyup=qty(<?= $x ?>) onchange=qty(<?= $x ?>) name='qty[]' id='qty_<?= $x ?>' value='<?= $dt->qty ?>'>
                                                        </td>
                                                        <td>
                                                            <input type='text' class='rm form-control' onkeyup='set_amount(<?= $x ?>)' name='rate[]' id='rate_<?= $x ?>' value="<?= $dt->rate ?>" style="pointer-events: none;">
                                                            <input type='hidden' class='rm form-control' readonly id='rateb_<?= $x ?>' style='pointer-events: none;' value="<?= $dt->buying_price ?>">
                                                            <input type='hidden' class='rm form-control' readonly id='ratec_<?= $x ?>' name='buying_price[]' style='pointer-events: none;' value="<?= $dt->buying_price ?>">
                                                        </td>
                                                        <td>
                                                            <input type='text' class='rm form-control tax' name='tax[]' id='tax_<?= $x ?>' style='pointer-events: none;' value="<?= $dt->tax_id ?>">
                                                            <input type='hidden' readonly class='rm form-control' id='taxrate_<?= $x ?>' style='pointer-events: none;' value="<?= $dt->tax_id ?>">
                                                        </td>
                                                        <td><input type='text' class='rm form-control total' name='amount[]' id='amount_<?= $x ?>' style='pointer-events: none;' value="<?= $dt->amount ?>"></td>
                                                        <td>
                                                            <button type="button" disabled class="btn btn-xs" title="Disabled"> <i class="fa fa-minus-circle"></i> </button>
                                                        </td>
                                                    </tr>
                                                <?php
                                                }
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 row-rg">
                                        <label>Tax: </label>
                                        <input type="text" readonly name="itemstax" id="itemstax" value="<?= $order_profile->tax_charge ?>" class="money money2 form-control">
                                    </div>
                                    <div class="col-md-4 row-rg">
                                        <label>Total: </label>
                                        <input type="text" readonly name="total" id="itemstotal" value="<?= $order_profile->net_amount ?>" class="money money2 form-control">
                                    </div>
                                    <div class="col-md-4 row-rg">
                                        <label>Counter/Table </label>
                                        <select class="form-control money2" name="eating_table_id" required>
                                            <?php
                                            foreach ($eating_tables as $eat) {
                                            ?>
                                                <option value="<?php echo $eat->eating_table_id; ?>" <?php if ($order_profile->eating_table_id == $eat->eating_table_id) echo 'selected'; ?>> <?php echo $eat->eating_table_name; ?> </option>
                                            <?php
                                            }
                                            ?>
                                        </select>
                                    ...
                                    <div class="col-md-4" hidden>
                                        ...
                                        <input type="number" name="paid_amount" id="paid_amount" value="" onkeyup="get_change_return(this)" class="money paid_amount form-control" autofocus="false" />
                                    </div>
                                    <div class="col-md-4" hidden>
                                    ...
                                    <div class="col-md-12 pt-3">
                                        <p style="padding-right: 10px" id="">
                                            <button type="submit" class="btn btn-app bg-primary btn-lg pull-right btnapp-rg " id="submit" title="Submit Data"><i class="fa fa-check-circle-o"></i> Update</button>

                                            <a href="#" onclick="window.open('<?= site_url('business/sales_receipt/' . $order_profile->order_id) ?>','receipt','width=400')" class="btn btn-app bg-primary btn-lg pull-right btnapp-rg " title="Print Receipt"><i class="fa fa-print"></i> Print</a>
                                            ...
                                            <a href="<?= site_url('posale/home'); ?>" class="btn btn-app bg-orangered btn-lg pull-left btnapp-rg " title="Close this sale window?"><i class="fa fa-close"></i> Close</a>
                                        </p>
                                    </div>
                                            ...
                                            <div class="thumbnail" style="height: 180px !important;">
                                                <button class="wrapper" onclick="add_row(<?php echo $itm->item_id; ?>)" <?php if ($itm->available_qty < 1) echo 'disabled'; ?> style="background: none; border: none;">
                                                    ...
                                                    <div class="caption" style="background-color: transparent !important;">
                                                        <small><?php echo $itm->item_name . ' - ' . $itm->barcode; ?></small>
                                                        <br />
...
<script type="text/javascript">
    function add_row(e) {
        ...
        $("#order_table tr:last").after("<tr id='row" + $rowno + "' >  " +
            "<td class='td'> " +
            "<input type='hidden' name='is_editted[]' value='1' readonly required /> " +
            "<input type='hidden' name='print_session[]' value='<?= $print_session ?>' readonly required />" +
            ...
            "<td> " +
            "<input type='text' class='rm form-control' onkeyup='set_amount(" + $rowno + ")' name='rate[]' id='rate_" + $rowno + "' style='pointer-events: none;' > " +
...
<script>
    $(function() {
        $('#place_order_form').on('submit', function(e) {
            ...
            $('#submit').html('').html('Wait while updating <i class="fa fa-spinner"></i>');
            $("#submit").attr("disabled", true);

            var orgtype = '<?=$_SESSION['organizationtype']?>';

            $.ajax({
                url: '<?php echo site_url('business/update_sale_order/' . $order_profile->order_id); ?>', //this is the submit URL
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    if (obj.resp == '1') {
                        ...
                        window.open("<?php echo site_url('business/sales_receipt/' . $order_profile->order_id); ?>", "receipt", "width=400, height=800");
                        ...
                        $('#submit').html('').html('Update <span class="fa fa-check-circle-o"></span>');
